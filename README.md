...
Collected knowledge, good practices and ideoms in the actual javascript language, building on top of the other in a somewhat cronological order.
...
```
```javascript
'foo' + 'bar' // faster for few substrings
['foo', 'bar'].join('') // faster for many substrings
```
```javascript
...
- `var`s which are not explicitly initialized are given the value `undefined`.
...
- Do not use `for in` with arrays (Actually, don't use `for in` at all, see below).
...
- See below section for templates of using typescript with angular 1.x
